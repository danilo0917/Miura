...
contract MIURAToken is BEP20, MinterRole {
    ...
    uint256 public sellFee = 10;   // default 5%
    uint256 public minSwapAmount = 100000000;   //0.1 MIURA tokens
    ...
    /**z
     * @notice Constructs the MIURA token contract.
     */
    constructor(address _treasury) public BEP20("MIURA", "MIURA") {
        operator = _msgSender();
    ...
    /// @dev overrides transfer function to meet tokenomics of MIURA
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "MIURA::transferOperator: new operator is the zero address");
    ...
    function updateBuyFee(uint16 _buyFee) public onlyOperator {
        require( _buyFee <= 100, "MIURA:: Biu Fee can't exceed 100%" );
        emit UpdateBuyFee(_buyFee);
        buyFee = _buyFee;
    }

    function updateSellFee(uint16 _sellFee) public onlyOperator {
        require( _sellFee <= 100, "MIURA:: Sell Fee can't exceed 100%");
    ...
    function updateFeeWallet(address _treasury) public onlyOperator {
        require( _treasury != address(0), "MIURA:: tax fee address is zero" );
    ...
    function updateLpcontract(address _lpcontract) public onlyOperator {
        require( _lpcontract != address(0), "MIURA:: lpcotontract address is not zero" );
